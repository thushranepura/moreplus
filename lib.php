 ...
 * the {@link theme_moreplus_less_variables()} here because we need to create selectors or
 * alter existing ones.
 *
 * @param theme_config $theme The theme config object.
 * @return string Raw LESS code.
 */
function theme_moreplus_extra_less($theme) {
    $content = '';
 ...
 */
function theme_moreplus_less_variables($theme) {
    $variables = array();
 ...
 */
function theme_moreplus_process_css($css, $theme) {

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_moreplus_set_logo($css, $logo);
    ...
    $css = theme_moreplus_set_customcss($css, $customcss);
 ...
 */
function theme_moreplus_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_moreplus_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM && ($filearea === 'logo' || $filearea === 'backgroundimage'
          || $filearea === 'carousel_slide1' || $filearea === 'carousel_slide2' || $filearea === 'carousel_slide3'  || $filearea === 'login_featurette_image'
            )) {
        $theme = theme_config::load('moreplus');
 ...
 */
function theme_moreplus_set_customcss($css, $customcss) {
...
function theme_moreplus_page_init(moodle_page $page) {
